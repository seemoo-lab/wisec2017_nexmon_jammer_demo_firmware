...
#define MONITOR_JAMMING_RECEIVER (1 << 6) // 64
...
void
send_udp_frame(struct wl_info *wl, uint32 timestamp, uint16 port, bool fcs_error) {
   struct osl_info *osh = wl->wlc->osh;
    struct sk_buff *p_new;
    struct jamming_receiver_header *jrh;

    p_new = pkt_buf_get_skb(osh, sizeof(struct ethernet_ip_udp_header) + sizeof(struct jamming_receiver_header));

    if (!p_new) return;

    skb_pull(p_new, sizeof(struct ethernet_ip_udp_header));
    jrh = (struct jamming_receiver_header *) p_new->data;
    jrh->timestamp = timestamp;
    jrh->node = 0xffff;
    jrh->port = port;
    jrh->fcs_error = fcs_error;
    jrh->length = 1000;
    jrh->encoding = 0;
    jrh->bandwidth = 0;
    jrh->rate = 0;
...
void
send_udp_frame_ext(struct wl_info *wl, struct wl_rxsts *sts, struct sk_buff *p)
{
    struct osl_info *osh = wl->wlc->osh;
    struct sk_buff *p_new;
    struct jamming_receiver_header *jrh;
    char *frame = (char *) (p->data + PLCP_LEN);
    ...
    skb_pull(p_new, sizeof(struct ethernet_ip_udp_header));
    jrh = (struct jamming_receiver_header *) p_new->data;
    jrh->timestamp = sts->mactime;
    jrh->node = *(uint16 *) &frame[4+6+6+4];
    jrh->port = ntohs(*(uint16 *) &frame[56]);
    jrh->fcs_error = sts->pkterror & WL_RXS_CRC_ERROR;
    jrh->length = p->len - PLCP_LEN;
    jrh->encoding = sts->encoding;
    jrh->bandwidth = sts->bw;
    jrh->rate = sts->datarate == 0 ? sts->mcs : sts->datarate;
    jrh->ldpc = (sts->htflags & WL_RXS_HTF_LDPC) != 0;
    prepend_ethernet_ipv4_udp_header(p_new);
    wl->dev->chained->funcs->xmit(wl->dev, wl->dev->chained, p_new);
}

static void
wl_monitor_jamming_receiver(struct wl_info *wl, struct wl_rxsts *sts, struct sk_buff *p) {
    char *frame = (char *) (p->data + PLCP_LEN);
    
    // Checking for correct frametype, mac addresses and UDP protocol setting
    if ((p->len > 60) && (frame[0] == 0x88) && (frame[43] == 0x11)) {
        send_udp_frame_ext(wl, sts, p);
    }
...
wl_monitor_hook(struct wl_info *wl, struct wl_rxsts *sts, struct sk_buff *p) {
    ...
    if (monitor & MONITOR_JAMMING_RECEIVER) {
        wl_monitor_jamming_receiver(wl, sts, p);
    }
}
