...
gen/ucode.asm: $(FW_PATH)/ucode.bin
	...
	$(Q)cat $@ | $(CC)gcc -fpreprocessed -dD -E - > tmp && mv tmp $@
	$(Q)gsed -i '/^$$/d' $@
	$(Q)gsed -i '/"<stdin>"/d' $@
	$(Q)gsed -i -r 's|(mov )(0x48)(, SPR_TME_VAL12)|\1MAC_SUBTYPE_DATA_NULL\3|' gen/ucode.asm
	$(Q)gsed -i -r 's|(mov )(0xC4)(, SPR_TME_VAL12)|\1MAC_SUBTYPE_CONTROL_CTS\3|' gen/ucode.asm
	$(Q)gsed -i -r 's|(mov )(0xD4)(, SPR_TME_VAL12)|\1MAC_SUBTYPE_CONTROL_ACK\3|' gen/ucode.asm
